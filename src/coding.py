...
@login_required
def add_train():
    train_name = request.form['Bus_name']
    ...
    qry = "INSERT INTO buses VALUES (null,%s, %s, %s, %s, %s, %s,%s)"
    val = (Bus_name,source_station, Destination_station, Departure_time, Arrival_time, Total_seats,Date)
    iud(qry, val)
...
def edit_train(train_id):
    ...
    qry = "SELECT * FROM buses WHERE bus_id = %s"
    res = selectone(qry, (bus_id))  # Assuming 'selectone' is a function that retrieves a single record

    if res is not None:
        # Render the edit_train.html template and pass the train details to it
        return render_template('update_train.html', val=res)
    else:
        return '''<script>alert("Bus not found");window.location="/manage_trains"</script>'''
...
def update_train():
    ...
    # Update query
    qry = "UPDATE buses SET Bus_name=%s,source=%s, destination=%s,departure_time=%s, arrival_time=%s,seats=%s,date=%s WHERE bus_id=%s"
    val = (Bus_name,source_station,destination_station,departure_time, arrival_time, total_seats,date,bus_id)
    iud(qry, val) 
    return '''<script>alert("Bus details updated successfully");window.location="/manage_buses"</script>'''
...
def manage_bookings():
    qry = """
        ...
        JOIN buses t ON b.bus_id = t.bus_id
    """
...
def update_profile():
    ...
    qry = "UPDATE users SET name=%s, username=%s, email=%s, password=%s WHERE id_users=%s"
...
def view_train():
    ...
    qry = "SELECT * FROM buses WHERE seats > 0"  # Adjust this query based on your database schema
...
def book_train():
    # try:
        ...
        train_id = request.args.get('bus_id')
        ...
        print("Train ID:", bus_id)  # This will help in debugging
        ...
        qry = "UPDATE buses SET seats = seats - %s WHERE bus_id = %s AND seats >= %s"
        ...
        qry2 = "INSERT INTO bookings_table (login_id, bus_id, passenger_name, source_station, destination_station, seats_booked) VALUES (%s, %s, %s, %s, %s, %s)"
        iud(qry2, (login_id, train_id, passenger_name, source_station, destination_station, seats_booked))
        return '''<script>alert("Bus booked successfully!");window.location="/view_train"</script>'''
...
def booking_history():
    ...
    qry = """
        SELECT b.booking_id, b.passenger_name, b.source_station, b.destination_station, b.seats_booked, t.Bus_name FROM bookings_table b JOIN buses t ON b.bus_id = t.bus_id WHERE b.login_id = %s
    """
...
def cancel_booking():
    ...
    qry1 = "SELECT bus_id, seats_booked FROM bookings_table WHERE booking_id = %s"
    booking_details = selectone(qry1, (booking_id,))
    
    if booking_details:
        train_id = booking_details['bus_id']
        seats_to_return = booking_details['seats_booked']
        
        # Update the seats count in the trains table
        qry2 = "UPDATE buses SET seats = seats + %s WHERE bus_id = %s"
